                  ...
                  {inboxEntries.map((item, idx) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        {hasPhotos && (
                          <div className="flex items-center gap-2 overflow-x-auto no-scrollbar py-1 mb-2.5">
                            {imagesList.slice(0, 4).map((imgUrl, i) => (
                              <div
                                key={`diary-preview-img-${item.id}-${i}`}
                                onClick={(e) => {
                                  e.stopPropagation();
                                  setLightboxSrc(imgUrl);
                                }}
                                className="relative w-16 h-16 sm:w-20 sm:h-20 shrink-0 rounded-xl overflow-hidden border border-neutral-700/30 group/img shadow-2xs"
                              >
                                <img
                                  src={imgUrl}
                                  alt={`Attachment ${i + 1}`}
                                ...
                                <div className="absolute inset-0 bg-black/0 group-hover/img:bg-black/20 flex items-center justify-center transition-colors">
                                  <Maximize2 className="w-3.5 h-3.5 text-white opacity-0 group-hover/img:opacity-100 transition-opacity" />
                                </div>
                              </div>
                            ))}
                            {imagesList.length > 4 && (
                              <div
                                className={`w-16 h-16 sm:w-20 sm:h-20 shrink-0 rounded-xl flex items-center justify-center text-xs font-semibold border ${
                                  isDark
